...
const Product = ({product}) => {
    ...
    const p = locale === 'en' ? product.data.translations[0] : locale === 'ru' ? product.data.translations[1] : locale === 'tm' ? product.data.translations[2] : ''
...
export async function getStaticProps({params}){
    const { id } = params
    const res = await fetch(`http://localhost:8055/items/product/${id}?fields=id,banner.*,gallery.*,translations.*`)
...
export async function getStaticPaths({locales}){
    const res = await fetch(`http://localhost:8055/items/product?fields=id,banner.*,gallery.*,translations.*`)
    const product = await res.json()

    const paths = []

    for(const locale of locales){
        product.data.map(item => paths.push({params : { id: item.id}, locale: locale}))
    }
